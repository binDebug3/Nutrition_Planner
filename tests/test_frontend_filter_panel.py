...
def test_render_dietary_toggles_bulk_sets_all_any_on(
    ...
    """Set every nutrient apply toggle to False through the remove action."""
    filter_panel_module = _load_filter_panel_module(load_module)
    state_manager_module = _load_state_manager_module(load_module)
    models_module = _load_models_module(load_module)
    manager = state_manager_module.NutrientStateManager(
...
def test_render_dietary_toggles_bulk_sets_all_any_off(
    ...
    """Set every nutrient apply toggle to True through the apply-all action."""
    filter_panel_module = _load_filter_panel_module(load_module)
    state_manager_module = _load_state_manager_module(load_module)
    models_module = _load_models_module(load_module)
    manager = state_manager_module.NutrientStateManager(
    ...
    for spec in models_module.NUTRIENT_SPECS:
        fake_streamlit.session_state[f"{spec.key}_any"] = False
...
def test_render_nutrient_filter_disables_manual_controls_when_apply_off(
    load_module: object,
    fake_streamlit: ModuleType,
) -> None:
    """Disable min/max and slider controls when nutrient requirements are off."""
    filter_panel_module = _load_filter_panel_module(load_module)
    state_manager_module = _load_state_manager_module(load_module)
    models_module = _load_models_module(load_module)
    manager = state_manager_module.NutrientStateManager(
    ...
    assert all(bool(call["disabled"]) is True for call in protein_control_calls)
    assert len(protein_slider_calls) == 1
    assert bool(protein_slider_calls[0]["disabled"]) is True


def test_render_nutrient_filter_enables_manual_controls_when_apply_on(
    load_module: object,
    fake_streamlit: ModuleType,
) -> None:
    """Keep min/max and slider controls enabled when nutrient requirements are on."""
    filter_panel_module = _load_filter_panel_module(load_module)
    state_manager_module = _load_state_manager_module(load_module)
    models_module = _load_models_module(load_module)
    manager = state_manager_module.NutrientStateManager(
